...
Before releasing to production, **YOU MUST** change these development values back to production values:

### 1. Meaningful Interaction Duration (2.5 minutes)
- [ ] `P2PConnectivityService.swift` (line ~31): Change from `30` to `150` seconds
- [ ] `MeaningfulInteractionView.swift` (line ~7): Change from `30` to `150` seconds

### 2. Convergence Timer Duration (5 minutes)
- [ ] `ConvergenceView.swift` (line ~7): Change from `60` to `300` seconds  
- [ ] `ConvergenceView.swift` (line ~41): Change divisor from `60.0` to `300.0`

> All locations have TODO comments in the code to remind you!
...
- **Dependencies:** Swift Package Manager to manage all third-party libraries
...
#### c. Meaningful Interaction Threshold:
...
#### d. "The Convergence" - The In-Person Invitation:
...
#### f. The Digital Handshake - Proving the Meetup:
...
### Phase 2: The Blind Gatekeeper & Core Loop
...
- [x] **Game Designer/Engineer:** Implement the Level 1 prompt system
- [x] **Lead Engineer/Game Designer:** Implement the "Meaningful Interaction" timer and message count logic
- [x] **Designer/Engineer:** Build the UI for "The Convergence" invitation flow, including the 30-second timer
